...
func TestCreateProduct(t *testing.T) {
	...
	payload := []byte(`{"name":"test products","price":11.22}`)
	...
	if m["name"] != "test products" {
		t.Errorf("Expected products name to be 'test product'. Got '%v'", m["name"])
	}
...
func addProducts(count int) {
	...
	for i := 0; i < count; i++ {
		a.DB.Exec("INSERT INTO products(name, price) VALUES($1, $2)", "Products "+strconv.Itoa(i), (i+1.0)*10)
	}
}
